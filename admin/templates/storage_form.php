<?php $title = empty($data) ? __('New File Storage') : __('Edit File Storage') ?>
<?php $breadcrumbs = [$return_url => __('File Storages'), '' => $title] ?>
...
<button onclick="if(confirm('<?=__('Delete an attribute?')?>')){var v=this.closest('.js-formfield-item');if(v){v.parentNode.removeChild(v)}}return false" style="background-color:transparent;border:none;" title="<?=__('Delete an attribute')?>" type="button"><img src="<?=$config['static_url']?>/assets/img/icon_cross_accent.svg"></button>
</div>
...
<button onclick="if(confirm('<?=__('Delete user permissions?')?>')){var v=this.closest('.js-formfield-item');if(v){v.parentNode.removeChild(v)}}return false" style="background-color:transparent;border:none;" title="<?=__('Delete user permissions')?>" type="button"><img src="<?=$config['static_url']?>/assets/img/icon_cross_accent.svg"></button>
</div>
...
<?php if (!empty($subfield['id'])) { ?><div class="col12"><div class="form-check"><input class="ays-ignore form-check-input" id="<?=e($subfield_id)?>-del" name="<?=e($subfield_id)?>-del" type="checkbox"><label class="form-check-label" for="<?=e($subfield_id)?>-del"><?=__('Delete')?></label></div></div><?php } else { ?><button onclick="if(confirm('<?=__('Delete an attribute?')?>')){var v=this.closest('.js-formfield-item');if(v){v.parentNode.removeChild(v)}}return false" style="background-color:transparent;border:none;" title="<?=__('Delete an attribute')?>" type="button"><img src="<?=$config['static_url']?>/assets/img/icon_cross_accent.svg"></button><?php } ?>
	</div>
<?php
	}
} ?>
...
<h5 style="margin:2rem 0 0;"><?=__('Users Permissions')?></h3>
...
<?php if (! empty($subfield['id'])) { ?><div class="col-12"><div class="form-check"><input class="ays-ignore form-check-input" id="<?=e($subfield_id)?>-del" name="<?=e($subfield_id)?>-del" type="checkbox"><label class="form-check-label" for="<?=e($subfield_id)?>-del"><?=__('Delete')?></label></div></div><?php } else { ?><button onclick="if(confirm('<?=__('Delete user permissions?')?>')){var v=this.closest('.js-formfield-item');if(v){v.parentNode.removeChild(v)}}return false" style="background-color:transparent;border:none;" title="<?=__('Delete user permissions')?>" type="button"><img src="<?=$config['static_url']?>/assets/img/icon_cross_accent.svg"></button><?php } ?>
	</div>
<?php
	}
} ?>
