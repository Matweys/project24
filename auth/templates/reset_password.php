...
<title><?=sprintf(__('%s - Set new password'), e($config['site_name']))?></title>
<link href="<?=$config['static_url']?>/auth/main.css?<?=$config['assets_ver']['auth'] ?? ''?>" rel="stylesheet" type="text/css">
</head>
<body>
<div class="wrapper">
	<h1 class="h3" style="font-weight:normal;margin:0 0 2rem;text-align:center;"><?=__('Set new password')?></h1>
...
<?php form_csrf_field($config, $csrf_id ?? '') ?>
		<div>
			...
			<input autofocus autocomplete="off" class="form-control" id="password" name="password" required type="password">
		</div>
		<button class="btn btn-primary" style="margin-top:1rem;width:100%;" type="submit"><?=__('Change password')?></button>
	</form>
