...
class Storage
{
    ...
    public function checkFilenameUnique(string $table, string $filename, ?int $parent_id = null)
    {
        $r = $this->db->prepare(
            "select id
from $table
where name = :name"
    ...
    public function deleteFileAndFolders(array $upload_config, int $storage_id, array $file_ids, int $max_files_to_delete = 0): array
    {
...
where
    ...
    and file.folder is not true'
                ...
                $r = $this->db->prepare(
                    "select file.id, file.file, file.image, file.name, folder.id as folder_id, folder.name as folder_name
from $file_table file
left join $file_table folder on folder.id = file.parent_id
join $file_table src on src.id in (" . str_repeat('?,', count($file_ids) - 1) . '?)
where
    ...
    public function deleteFiles(array $upload_config, int $storage_id, array $file_ids)
    {
        ...
        if ($storage) {
            ...
            if ($file_table) {
                ...
                $r = $this->db->prepare("select file.id, file.file, file.image, file.name, folder.id as folder_id, folder.name as folder_name from $file_table file left join $file_table folder on folder.id = file.parent_id where file.id in (" . str_repeat('?,', count($file_ids) - 1) . '?) and file.folder is not true');
                $r->execute($file_ids);
                $files = $r->fetchAll(\PDO::FETCH_ASSOC);
